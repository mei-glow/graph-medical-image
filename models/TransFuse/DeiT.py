...
def deit_small_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt = torch.load(Path(__file__).resolve().parent / 'pretrained' / 'deit_small_patch16_224-cd65a155.pth', map_location='cpu')
...
def deit_base_patch16_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        ckpt = torch.load(Path(__file__).resolve().parent / 'pretrained' / 'deit_base_patch16_224-b5f2ef4d.pth', map_location='cpu')
        model.load_state_dict(ckpt['model'], strict=False)
...
def deit_base_patch16_384(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        ckpt = torch.load(Path(__file__).resolve().parent / 'pretrained' / 'deit_base_patch16_384-8de9b5d1.pth', map_location='cpu')
