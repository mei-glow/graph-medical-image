...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load(pretrained_dir / 'resnet34-333f7ec4.pth', map_location='cpu'))
...
class TransFuse_L(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load(pretrained_dir / 'resnet50-19c8e357.pth', map_location='cpu'))
        self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()
