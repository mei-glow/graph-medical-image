...
class FAT_Net(nn.Module):
    def __init__(
        ...
        if deit_pretrained_path is not None:
            checkpoint_path = Path(deit_pretrained_path)
            if not checkpoint_path.exists():
                raise FileNotFoundError(f"DeiT pretrained checkpoint not found: {checkpoint_path}")
            ...
            if not isinstance(state, dict):
                raise TypeError("Unsupported DeiT checkpoint format for FAT-Net")

            cleaned_state = {}
            for key, value in state.items():
                new_key = key.replace("module.", "", 1) if key.startswith("module.") else key
                if new_key.startswith("deit."):
                    new_key = new_key.replace("deit.", "", 1)
                cleaned_state[new_key] = value
            transformer.load_state_dict(cleaned_state, strict=False)
        elif pretrained_backbone:
            raise ValueError(
                "FAT-Net requires a local DeiT pretrained checkpoint path; automatic hub download is disabled."
            )

        resnet = resnet_model.resnet34(weights=None)
        if resnet_pretrained_path is not None:
            checkpoint_path = Path(resnet_pretrained_path)
            if not checkpoint_path.exists():
                raise FileNotFoundError(f"ResNet34 pretrained checkpoint not found: {checkpoint_path}")

            state = torch.load(checkpoint_path, map_location="cpu")
            if isinstance(state, dict) and "state_dict" in state:
                state = state["state_dict"]
            if not isinstance(state, dict):
                raise TypeError("Unsupported ResNet34 checkpoint format for FAT-Net")
            ...
            for key, value in state.items():
                ...
                cleaned_state[new_key] = value
            resnet.load_state_dict(cleaned_state, strict=False)
        elif pretrained_backbone:
            raise ValueError(
                "FAT-Net requires a local ResNet34 pretrained checkpoint path; automatic torchvision download is disabled."
            )
